...
coreTeamRouter.post(
    ...
    async (req, res) => {
        try {
            ...
            if (!memberImageFile?.buffer) {
                return res
                    .status(400)
                    .json({ message: "Member Image is Required!!" });
            }
            const memberImageCloudinary = await uploadOnCloudinary(
                memberImageFile.buffer,
                {
                    folder: "core-team",
                    public_id: `${Date.now()}-${memberImageFile.originalname}`,
                },
            );

            const teamData = await coreTeamModel.create({
                photoUrl:
                    memberImageCloudinary.secure_url ||
                    memberImageCloudinary.url,
                fullName,
                designation,
                isActive,
            });
...
coreTeamRouter.delete(
    "/admin/coreteam/delete/:_id",
    adminAuth,
    async (req, res) => {
        try {
            const { _id } = req.params;
            ...
            const updatedData = await coreTeamModel.findByIdAndDelete(_id);

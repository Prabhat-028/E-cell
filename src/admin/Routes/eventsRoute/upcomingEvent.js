...
eventRouter.post(
    ...
    async (req, res) => {
        try {
            ...
            const upcomingEventsBannerFile =
                req.files?.upcomingEventsBanner?.[0];
            if (!upcomingEventsBannerFile?.buffer) {
                return res
                    .status(400)
                    .json({
                        message: "Banner of the upcoming event is required",
                    });
            }
            const upcomingEventCloudinary = await uploadOnCloudinary(
                upcomingEventsBannerFile.buffer,
                {
                    folder: "upcoming-events",
                    public_id: `${Date.now()}-${upcomingEventsBannerFile.originalname}`,
                },
            );
            const eventData = new eventsModel({
                photoUrl:
                    upcomingEventCloudinary.secure_url ||
                    upcomingEventCloudinary.url,
                scheduledDate,
                eventName,
                speaker,
            });
...
//deleting an event
eventRouter.delete("/admin/upcomingevent/:_id", adminAuth, async (req, res) => {
    try {
